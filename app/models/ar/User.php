...
class User extends ActiveRecord implements IdentityInterface
{
    ...
    {
        if ($this->_device_ids === null) {
            $this->_device_ids = $this->getDevices()->select(['id'])->column();
        }
        return $this->_device_ids;
    }

    public function fcmSend($data)
    {
        $ids = Device::find()->select(['notive_key'])
    ...
    public function pusherSend($event, $data)
    {
        $channel = 'sub_user_' . $this->id;
        Yii::$app->queue->push('notive/pusher-push', [$channel, $event, $data]);
    }
}

...
class TokenManager extends Object
{
    ...
    public function generateToken($data, $category = 'app', $expire = 0, $unique = false)
    {
        $token = md5(serialize([Yii::$app->params['app.secretKey'], $category, $data, $unique ? time() : false]));
        $query = (new Query)->from($this->tokenTable)->where(['id' => $token, 'category' => $category]);
        if ($query->exists($this->db)) {
            $command = $this->db->createCommand()->update($this->tokenTable, [
                'expire' => $expire > 0 ? time() + $expire : 0,
                ], ['id' => $token, 'category' => $category]);
        } else {
